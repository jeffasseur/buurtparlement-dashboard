...
?> 
    <div class="calendar-contain"> 
        ...
        <div class="calendar__sidebar" id="event_list"> 
            <?php echo getEvents(); ?> 
        </div> 
         
        <section class="calendar__days"> 
            <section class="calendar__top-bar"> 
                <span class="top-bar__days">Ma</span> 
                <span class="top-bar__days">Din</span> 
                <span class="top-bar__days">Woe</span> 
                <span class="top-bar__days">Do</span> 
                <span class="top-bar__days">Vr</span> 
                <span class="top-bar__days">Za</span> 
                <span class="top-bar__days">Zo</span> 
            </section> 
            ...
            <?php  
                ...
                for($cb=1;$cb<=$boxDisplay;$cb++){ 
                    if(($cb >= $currentMonthFirstDay || $currentMonthFirstDay == 1) && $cb <= ($totalDaysOfMonthDisplay)){ 
                        ...
                        $statement = $conn->prepare("select count(*) from events where date = '".$currentDate."' and status = 1");
                        $statement->execute();
                        $eventNum = $statement->fetchColumn(); 
...
function getEvents($date = ''){ 
    ...
    $eventListHTML = '<h5 class="sidebar__heading">'.date("l", strtotime($date)).'<br>'.date("F d", strtotime($date)).'</h5>'; 
     
    // Fetch events based on the specific date 
    $conn = DB::getConnection();
    $statement = $conn->prepare("SELECT title FROM events WHERE date = '".$date."' AND status = 1");
    ...
    if($result > 0){ 
        ...
        foreach($row as $r){ 
            $i = 0;
            $eventListHTML .= '<li class="sidebar__list-item"><span class="list-item__time">'.$i.'.</span>'.$r['title'].'</li>'; 
            $i++;
        } 
        $eventListHTML .= '</ul>'; 
    } 
